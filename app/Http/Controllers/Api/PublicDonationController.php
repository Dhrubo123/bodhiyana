...
class PublicDonationController extends Controller {
    ...
    public function event(Event $event): JsonResponse { abort_unless($event->is_active, 404); return response()->json($event); }
    public function eventAsset(string $path): StreamedResponse { abort_unless(str_starts_with($path, 'events/') && Storage::disk('public')->exists($path), 404); return Storage::disk('public')->response($path, null, ['Cache-Control' => 'public, max-age=31536000, immutable']); }
