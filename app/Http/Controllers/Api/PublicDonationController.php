...
class PublicDonationController extends Controller {
    ...
    public function websiteSettings(): JsonResponse { $settings = WebsiteSetting::pluck('value', 'key')->all(); return response()->json(['bihar_name'=>$settings['bihar_name']??'আপনার বৌদ্ধ বিহার', 'site_title'=>$settings['site_title']??$settings['bihar_name']??'বৌদ্ধ বিহার | দান ব্যবস্থাপনা', 'logo_url'=>$this->websiteAssetUrl($settings['logo_path']??null), 'favicon_url'=>$this->websiteAssetUrl($settings['favicon_path']??null)])->header('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0'); }
